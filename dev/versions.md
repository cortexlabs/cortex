...
1. Update the versions listed in "Pre-installed packages" in `realtimeapi/predictors.md` and `batchapi/predictors.md` (look at the diff carefully since some packages are not shown, and e.g. `tensorflow-cpu` -> `tensorflow`)
