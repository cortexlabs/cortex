...
1. Copy the charts containing the istio crds, istio pilot and istio ingress gateway into helm/charts/networking/charts. As of 1.7.3 these charts are in folders named: `base`, `istio-control/istio-discovery`, `gateways/istio-ingress`. Copy the istio-ingress folder twice except name one of them api-ingress and the other operator-ingress.
1. Update helm/charts/networking/values.yaml to override globals and default values.yaml in the istio charts as necessary
...
1. Test the helm charts for both aws and gcp provider `helm template testing helm -n default --dry-run -f <values.yaml>` and verify that none of the resources are namespaced to any istio namespaces.
