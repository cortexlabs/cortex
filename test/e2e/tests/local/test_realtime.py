...
from e2e.expectations import assert_response_expectations, parse_expectations
from e2e.utils import apis_ready, request_prediction

TEST_APIS_DIR = Path(__file__).parent.parent.parent.parent / "apis"
...
def test_realtime_apis(client: cx.Client, api: str):
    ...
    try:
        assert apis_ready(client=client, api_names=api_names), f"apis {api_names} not ready"
