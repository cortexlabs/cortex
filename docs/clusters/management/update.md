...
```bash
# download the desired CLI version, replace 0.40.0 with the desired version (Note the "v"):
...
```bash
# download the desired CLI version, replace 0.40.0 with the desired version (Note the "v"):
CORTEX_INSTALL_PATH=$(pwd)/cortex0.40.0 bash -c "$(curl -sS https://raw.githubusercontent.com/cortexlabs/cortex/v0.40.0/get-cli.sh)"

# confirm cortex CLI version
./cortex0.40.0 version
```
