...
When `max_batch_size` and `batch_interval` fields are set for the [TensorFlow Predictor](../deployments/syncapi/predictors.md#tensorflow-predictor), errors can be encountered if the associated model hasn't been built for batching.
