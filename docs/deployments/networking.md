...
You can configure your API to be private. If you do this, you must use [VPC Peering](../aws/vpc-peering.md) to connect to your APIs.

The SSL certificate on the API load balancer is autogenerated during installation using `localhost` as the Common Name (CN). Therefore, clients will need to skip certificate verification when making HTTPS requests (e.g. `curl -k`). Alternatively, you can set up a [custom domain](../aws/custom-domain.md), which will use ACM to provision SSL certs for your domain.
