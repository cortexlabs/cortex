...
func promptIfNotAdmin(awsClient *aws.Client, disallowPrompt bool) {
	...
	if !awsClient.IsAdmin() {
		warningStr := fmt.Sprintf("warning: your IAM user%s does not have administrator access. Please visit https://docs.cortex.dev/v/%s/ to make sure that your credentials have the minimum iam policies.\n\n", accessKeyMsg, consts.CortexVersionMinor)
		if disallowPrompt {
