	...
	if len(apisRequiringGPU) > 0 {
		...
		if _, ok := infoResponse.Runtimes["nvidia"]; !ok {
			if !strings.HasPrefix(strings.ToLower(runtime.GOOS), "linux") {
				fmt.Printf("warning: %s will run without gpu access because the nvidia container runtime is not supported on your operating system; see https://cortex.dev/v/%s/troubleshooting/nvidia-container-runtime-not-found for more information\n\n", s.StrsAnd(apisRequiringGPU.Slice()), consts.CortexVersionMinor)
			} else {
