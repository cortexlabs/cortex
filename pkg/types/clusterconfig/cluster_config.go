...
func (cc *Config) Validate(awsClient *aws.Client) error {
	...
	if err := awsClient.VerifyNetworkQuotas(1, cc.NATGateway != NoneNATGateway, cc.NATGateway == HighlyAvailableNATGateway, requiredVPCs, strset.FromSlice(cc.AvailabilityZones), len(cc.NodeGroups)); err != nil {
		// Skip AWS errors, since some regions (e.g. eu-north-1) do not support this API
		if !aws.IsAWSError(err) {
			return err
		}
