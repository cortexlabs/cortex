...
func fetchLogsFromCloudWatch(jobStatus *status.TaskJobStatus, podCheckCancel chan struct{}, socket *websocket.Conn) {
...
func streamFromCloudWatch(jobStatus *status.TaskJobStatus, podCheckCancel chan struct{}, socket *websocket.Conn) {
	jobKey := jobStatus.JobKey
	jobSpec := jobStatus.TaskJob
	logGroupName := logGroupNameForJob(jobStatus.JobKey)
