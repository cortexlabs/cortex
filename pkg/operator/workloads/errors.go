...
func ErrorNoAvailableNodeComputeLimit(resource string, req, max int64) error {
	return Error{
		...
		message: fmt.Sprintf("there are no available nodes that can satisfy the requested %s resource quantity - requested %d but nodes only have %d", resource, req, max),
	}
}
