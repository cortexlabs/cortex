...
func (config *Config) Validate(envName string) error {
	...
	for _, model := range config.Models {
		if !slices.IsStrInSlice(model.TargetColumn, columnNames) {
	...
	for _, api := range config.APIs {
		if !slices.IsStrInSlice(api.ModelName, modelNames) {
	...
	for _, aggregate := range config.Aggregates {
		if !strings.Contains(aggregate.Aggregator, ".") && !slices.IsStrInSlice(aggregate.Aggregator, aggregatorNames) {
	...
	for _, transformedColumn := range config.TransformedColumns {
		if !strings.Contains(transformedColumn.Transformer, ".") && !slices.IsStrInSlice(transformedColumn.Transformer, transformerNames) {
