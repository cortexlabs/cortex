...
def get_models_from_api_spec(
    ...
    for model in models:
        ...
        ends_as_file_path = model["model_path"].endswith(".onnx")
        if ends_as_file_path and os.path.exists(
            os.path.join(
                model_dir, model_resource["name"], "1", os.path.basename(model["model_path"])
            )
        ):
            ...
            model_resource["model_path"] = os.path.join(
                model_dir, model_resource["name"], "1", os.path.basename(model["model_path"])
            )
