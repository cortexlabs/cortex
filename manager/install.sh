...
function cluster_up() {
  ...
  echo -n "￮ updating bucket lifecycle policies "
  setup_bucket_policy
  echo "✓"
...
function setup_bucket_policy() {
  python render_template.py manifests/async-bucket-lifecycle.json.j2 > manifests/async-bucket-lifecycle.json
  aws s3api put-bucket-lifecycle --bucket $CORTEX_BUCKET --lifecycle-configuration file://manifests/async-bucket-lifecycle.json
}
