...
Alternatively to [throughput_test.py](../../utils/throughput_test.py), the `ab` GNU utility can also be used to benchmark the API. This one has the advantage that it's not taxing on your dev box, but the disadvantage that it doesn't implement a cooldown period. You can run this like this.

```bash
ab -n <number-of-requests> -c <concurrency-level> -p sample.bin -T 'application/octet-stream' -rks 120 $ENDPOINT # for making octet-stream requests, which is the default for throughput_test script
ab -n <number-of-requests> -c <concurrency-level> -p sample.json -T 'application/json' -rks 120 $ENDPOINT # for making json requests, will take a hit on performance because the API has to download the image every time
```
